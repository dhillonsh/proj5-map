...
def process(raw):
    ...
    for line in raw:
        ...
        if field == "begin":
            ...
            print(arrow.utcnow().timestamp - date.timestamp)
            print('1' if (arrow.utcnow().timestamp - date.timestamp) < 604800 else '0')
            entry['currentWeek'] = '1' if (arrow.utcnow().timestamp - date.timestamp) < 604800 else '0'
